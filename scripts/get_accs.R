...
write.table(row.names = F, col.names = F, db[, c(accs_col, "BioSample.Accession")],
...
fetch_cmds <- paste0(
  ...
  'efetch -format docsum > ', file.path(ncbi_dir, db$BioSample.Accession)) # get the summary as XML
...
system(paste0("parallel -j ", cores, " :::: ", fetch_cmds_path))
...
max_lines = max(count.fields("~/GitHub/sraFind/test_direct/hits.txt", sep="\t", skip = 0,
                             blank.lines.skip = TRUE, comment.char = "#"))
min_lines = min(count.fields("~/GitHub/sraFind/test_direct/hits.txt", sep="\t", skip = 0,
                             blank.lines.skip = TRUE, comment.char = "#"))
table(count.fields("~/GitHub/sraFind/test_direct/hits.txt", sep="\t", skip = 0,
                 blank.lines.skip = TRUE, comment.char = "#"))
...
all_biosamples <- merge(db[, c(accs_col, "BioSample.Accession")], hits, by.x="BioSample.Accession", by.y="biosample")
